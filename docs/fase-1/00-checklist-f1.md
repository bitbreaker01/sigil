...
| 13 | Primeras Custom APIs (`CreateTransaction`, `UpdateDraft`, `DeleteDraft`, `GetDocumentContent`) — TDD | Claude | Suites M1/M7 (doc 11 §4) + test de conformidad de registro de las APIs en el ambiente | 🟡 Código y pruebas 2026-07-15 — **núcleo puro** (Domain/: Choices, SchemaNames, ReglasDeAutorizacion, ValidacionDeEntrada) con **88 tests verdes** (M1/M7); **cáscara net462** (4 plugins + stub artesanal de IOrganizationService + seam IFileTransfer) con sus tests (corren en CI Windows); **CF-D01..D04** de registro (rojos por diseño hasta F2). Antagonista aplicado (9 advertencias corregidas: falsos verdes del arnés A3/A4/A5, pérdida silenciosa de zonas A2, validación de Message, desalineaciones doc 04 §1/§3.4). **Falta despliegue en Dev (F2)** para poner CF-D en verde |
