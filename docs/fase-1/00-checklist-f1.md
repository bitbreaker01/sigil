...
| F2.4 | `VerifyDocument` + Jobs (`ExpireTransactions` T12+T14, `ProcessReminders`, `ResealPending`) — M10 | Claude | Suite M10 + CF-D02 con privilegio de SERVICIO por job + smokes CF-D11/D12/D13 | ✅ 2026-07-16 — **177 Core verdes** (M10 + verificación cruzada del historial); **CF-D11 VERDE: sellar real→verificar hash correcto VERDE→hash alterado ROJO→historial íntegro→evento 11**; CF-D12/D13 verdes. Package v1.0.11, **15 Custom APIs** + worker. **147/147 conformidad. DEUDA DEL ANTAGONISTA SALDADA (2026-07-16):** revisión completa aplicada — 1 crítico (cap de lote en ResealPending por presupuesto de 2 min), 9 advertencias corregidas (evento 11 sobre tx sin sellar; N+1 del creador cacheado; AnchorMismatchCount para ancla rota; CF-D03/D04 con las 14 filas de F2.4; hash mal formado = error de contrato; runbook D actualizado a 15 APIs; gaps de smoke T14/Reseal declarados en doc 11). Evento "TSA abandonada" (159460012) copiado del portal, en código + docs + CF-A16 |
