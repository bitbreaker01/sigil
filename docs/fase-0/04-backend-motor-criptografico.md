...
| Contratos JSON de las Custom APIs (§4) | **System.Text.Json 8.0.x** | MIT | Serialización de ParticipantsJson/ZonesJson/env vars. netstandard2.0: viaja como dependent assembly del package (mismo mecanismo que PDFsharp/BouncyCastle). ⚠️ **Pendiente de validar en sandbox net462** (arrastra System.Memory/Encodings.Web/Bcl.AsyncInterfaces) — agregar un caso de deserialización al spike `RunSpike` antes del primer push de F2 (precedente `XImage`: lo que corre en net8 puede fallar solo en el sandbox) |
