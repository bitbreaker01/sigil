...
| T9 | Sellando | Error de Sellado | Worker — fallo definitivo. **Precisión (2026-07-16):** los reintentos AGOTADOS de `OperationStatus.Retry` NO los transiciona el worker (tras el 4º retry la plataforma no lo ejecuta más) — esa transacción queda en Sellando hasta que el **saneamiento T14** la mueva. `capi_ExpireTransactions` (que implementa T14) está desplegada desde F2.4; hasta que el **flow diario** que la invoca exista (F4), correrla manualmente o por schedule externo (runbook D) | Sistema | — | Evento 8 con detalle accionable; visible con alerta en dashboard del creador (doc 05 §4.1) |
