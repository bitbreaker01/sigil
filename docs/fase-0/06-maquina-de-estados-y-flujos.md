...
| T9 | Sellando | Error de Sellado | Worker — fallo definitivo. **Precisión (2026-07-16):** los reintentos AGOTADOS de `OperationStatus.Retry` NO los transiciona el worker (tras el 4º retry la plataforma no lo ejecuta más) — esa transacción queda en Sellando hasta que el **saneamiento T14** (job diario, 24 h) la mueva. Hasta que `capi_ExpireTransactions` exista (F2.4), la salida es manual (runbook) | Sistema | — | Evento 8 con detalle accionable; visible con alerta en dashboard del creador (doc 05 §4.1) |
