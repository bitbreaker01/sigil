...
**Enmienda (2026-07-16, calibración de la implementación — M8):** el "contraste RMS por histograma" global se **redefinió** como *RMS del apartamiento de la tinta respecto del fondo blanco, sobre píxeles no transparentes*. Justificación: una firma legítima de trazo fino (5% de cobertura de tinta negra) da σ global ≈ √(0,05·0,95) ≈ 0,218 < 0,25 — el RMS global mide "cuánta tinta hay", no "qué tan oscura es"; la métrica implementada da ~1,0 para tinta negra y ~0,03 para trazos desvaídos, independiente de la cobertura, que es la intención del umbral. **Calibración pendiente:** la varianza del Laplaciano es dependiente de la resolución de origen (~1/lado; el umbral 80 está implícitamente calibrado a ~600×200) — calibrar con PNGs reales anti-aliaseados antes de endurecerlo. Además se agregó un **techo de dimensiones de entrada (4096×4096)**, verificado sobre el header ANTES de decodificar — anti bomba de descompresión en el sandbox.
