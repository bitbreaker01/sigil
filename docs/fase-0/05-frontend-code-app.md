...
**Decisión (Plan A):** personalizar la CSP del ambiente — mecanismo oficial (`PowerApps_CSPConfigCodeApps` / admin center, verificado que los valores custom **reemplazan** el default): **`worker-src 'self' blob:`**, **`child-src 'self' blob:`** y `connect-src 'self'`. *(Enmienda 2026-07-21: al adoptar el worker de pdf.js **inline como blob** — `?worker&inline` en F3 —, `worker-src` requiere `blob:`; sin él el visor no arranca. `blob:` no habilita orígenes externos: son URLs same-origin generadas en runtime.)* *(Enmienda 2026-07-22, hallazgo de producción: **`child-src 'self' blob:` es indispensable para Safari** — Safari NO soporta `worker-src` y cae en cascada a `child-src` (default `'none'`), así que en iPhone/iOS no carga NINGÚN PDF si falta. Chrome/Edge sí leen `worker-src`; de ahí que el bug aparezca por navegador/tenant.)* Esta configuración es **por ambiente** y entra al runbook de ALM (**doc 09 la hereda como requisito**: Dev/Test/Prod deben configurarla antes del primer deploy; se valida con el modo reporting de CSP primero).
...
|------|--------|-----------|
| CSP de ambiente personalizada (`worker-src 'self' blob:`, `child-src 'self' blob:`, `connect-src 'self'`) | **Requisito nuevo para doc 09** — verificado que el default bloquea el plan ingenuo; `child-src` es obligatorio por Safari | Plan B fake-worker + factories bundleadas (§6.1) |
