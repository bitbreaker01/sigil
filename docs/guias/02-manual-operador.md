...
La CSP del componente de la app viene **forzada por defecto en modo restrictivo**, lo que **bloquea el motor del visor de PDF** — es decir, el visor está roto en un ambiente nuevo **hasta que se configura**. Hay que ampliar **tres** directivas:
...
- **Directiva de *worker secundario* (`child-src`):** también `'self' blob:`. **Indispensable para Safari:** Safari no lee la directiva del worker y usa esta en su lugar; sin ella, **en iPhone/iOS no carga ningún PDF** aunque la del worker esté bien (los navegadores de escritorio sí funcionan — por eso el problema aparece solo con ciertos usuarios/dispositivos).
...
| **El visor de PDF no renderiza** / errores de seguridad en la consola | La **CSP** por defecto bloquea el motor del visor | Configurá las **tres** directivas (§6.2); replicá los valores exactos de Dev |
| **El visor NO carga en iPhone/Safari pero SÍ en Chrome/Edge** (o falla solo para algunos usuarios/tenants) | Falta la directiva **`child-src 'self' blob:`** — Safari no soporta la directiva del *worker* y cae en cascada a `child-src` (que por defecto está en `'none'`) | Agregá `child-src 'self' blob:` a la CSP del ambiente (§6.2). Leé el error exacto de la consola (`Refused to create a worker from 'blob:...'`) para confirmar qué directiva bloquea |
