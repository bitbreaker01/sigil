...
| PDF (abrir existente, dibujar PNG en coordenadas, agregar página) | **PDFsharp ≥ 6.2.4** (jamás 6.2.0) | MIT | Puramente managed, netstandard2.0; `XGraphics.FromPdfPage` + `AddPage` cubren ADR-011. iText descartado (AGPL); QuestPDF descartado (no edita PDFs existentes); Aspose/Syncfusion descartados (costo sin necesidad). **Transparencia validada por spike (2026-07-10): ver §10.** ⚠️ **Finding sandbox (2026-07-15):** `XImage.FromStream(png)` falla ("Unsupported image format") **solo** bajo net462 en el sandbox de Dataverse — el importer PNG retorna null silenciosamente (JPEG y BMP sí funcionan; el decoder interno BigGustave sí abre el PNG). El motor dibuja las imágenes con **XObject manual** (dict `/Image` + stream FlateDecode RGB + `/SMask` gris para el alfa + operador `Do` en el content stream) — validado en sandbox real con decode vía BigGustave; el motor usará **ImageSharp como decoder** (validado por separado en sandbox), la cadena combinada se valida con el primer test del motor. Ver §10 |
...
| Contratos JSON de las Custom APIs (§4) | **System.Text.Json 9.0.6** | MIT | Serialización de ParticipantsJson/ZonesJson/env vars. netstandard2.0: viaja como dependent assembly del package. **VALIDADO en sandbox real (spike v1.0.9, 2026-07-15):** (de)serialización reflexiva de los contratos reales (Guid, `int?`, double, `JsonPropertyName`, camelCase) + `JsonDocument` → OK en 481 ms; el trust parcial NO bloqueó la reflexión. **Finding (medido):** se shippeó assembly 8.0.0.5 y el sandbox corrió `9.0.0.6` en runtime. **Interpretación** (consistente, no instrumentada): la plataforma provee su propia 9.0 y unifica, como con `Microsoft.Xrm.Sdk`. → se pinea **9.0.6** (assets `lib/net462` confirmados) para parity test↔runtime. Ver §10 y `spikes/spike-sandbox/RESULTADOS-SANDBOX.md` |
...
- Nomenclatura según [12-convenciones-nomenclatura.md](12-convenciones-nomenclatura.md): Custom APIs = `sanic_sigil_capi_<VerboPascalCase>`; backing plugin síncrono salvo el worker (§7). `IsPrivate` **NO es control de acceso** (verificado: solo oculta del `$metadata`; cualquier usuario autenticado puede invocar una API si conoce su firma). El control real: **Execute Privileges (§3.2) + autorización en el plugin (§3.3)**.
  > **Reconciliación 04↔05 (2026-07-18):** la Fase 0 fijó `IsPrivate = true` como "higiene de metadata". Pero el frontend (doc 05) genera sus clientes tipados con `power-apps add-dataverse-api`, y los cloud flows (F4) resuelven la acción, **ambos leyendo del `$metadata`** — que `IsPrivate=true` oculta. Como `IsPrivate` no aporta seguridad (el control real no cambia), se resolvió a **`IsPrivate = false`** para habilitar la generación de clientes y los flows. Vigente en `Sigil.Deploy` y verificado por CF-D02.
...
| `sanic_sigil_capi_ResealPending` | Unbound (job) | ADR-005: reintenta TSA sobre ledgers en `Re-sellado pendiente`. **Si `sanic_sigil_env_TsaEnabled` = false, transiciona esos ledgers a `Sin sello TSA`** (evita huérfanos eternos bajo una etiqueta que promete un reintento que no va a ocurrir) **+ evento 13 "TSA abandonada"** (agregado 2026-07-16). **Cap de lote por corrida** (`MaxResellosPorCorrida`, default 3): la descarga + TSA + rate limit de Sectigo (~20-30 s/ledger) cruzaría el límite duro de 2 min con backlog; el job es diario e idempotente, drena de a poco. **Ancla rota** (archivo ≠ finalhash del ledger): jamás se sella, se cuenta en `AnchorMismatchCount` (señal de integridad catastrófica, distinta de "TSA caída"). Out: `ResealedCount`, `MovedToNoTsaCount`, `StillPendingCount`, `AnchorMismatchCount` |
...
- `PdfBase64`: longitud del string ≤ límite derivado de `sanic_sigil_env_MaxPdfSizeKB` **antes de decodificar** (magic bytes `%PDF-` **en el offset 0** — rechazo estricto: no se tolera basura antes de la cabecera); apertura exitosa con PDFsharp; **documento cifrado/protegido → rechazo**; **documento con firmas digitales previas → rechazo** con mensaje claro (incrustar imágenes las invalidaría — y una "firma sobre firma" es una ambigüedad probatoria que no aceptamos). Detección de firma previa: `/AcroForm` con `SigFlags` bit *SignaturesExist* o cualquier campo `/FT /Sig` con valor `/V` (PDF 32000-1 §12.7.4.5). Postura PDF/A: se acepta, advirtiendo que el sellado rompe la conformidad PDF/A estricta (el documento final es un PDF estándar).
...
**Spikes ejecutados el 2026-07-10** (.NET 8/Linux — evidencia en `spikes/RESULTADOS.md`) **y el 2026-07-15 dentro del sandbox real de Dataverse Dev** (plugin package net462, isolationMode=2, vía Custom API `sanic_sigil_capi_RunSpike` — evidencia en `spikes/spike-sandbox/RESULTADOS-SANDBOX.md`):

| Ítem | Estado | Resultado / Mitigación |
|------|--------|-----------|
| **PDFsharp + PNG con transparencia** (issue empira/PDFsharp#187) | **RESUELTO POR SPIKE — PASS con ajuste de diseño** | El issue NO se reproduce en **6.2.4** en .NET 8: `/SMask` presente y render verificado por píxeles. **PERO en el sandbox net462 el importer PNG de `XImage.FromStream` falla siempre** ("Unsupported image format"; bisecado exhaustivamente: header PNG correcto, BigGustave interno abre el archivo, JPEG/BMP funcionan por la misma vía). **Ajuste:** el motor incrusta imágenes con **XObject manual** (FlateDecode RGB + `/SMask` DeviceGray, zlib artesanal 0x78 0x9C + Adler32 porque net462 no tiene ZLibStream) — validado en sandbox real: PDF de 4.341 bytes, `/SMask` presente, round-trip `PdfReader` OK. Regla: **pin ≥ 6.2.4, jamás 6.2.0**; contrato de normalización PNG RGBA 8-bit no entrelazado (§4) sigue vigente |
| **BouncyCastle RFC 3161** | **RESUELTO POR SPIKE — PASS (sandbox real)** | En .NET 8: Granted en 815 ms, nonce verificado, **CertReq honrado (3 certs embebidos)**, `token.Validate()` OK, cruce con `openssl ts` OK. **En sandbox real (2026-07-15): Sectigo HTTP 200 en 544 ms, `token.Validate()` OK, token 6.633 bytes DER = 8.844 chars base64 → presupuesto de columna memo: ~12K chars** (doc 03) |
| **DigiCert inaccesible** (timeout de cliente sin respuesta HTTP; Sectigo respondió en 120–815 ms según corrida) | Finding de red, confirmado TAMBIÉN desde el sandbox | Timeout de 15 s desde la red del spike (TCP diagnosticado con curl) **y desde la red saliente del sandbox Dataverse** (TaskCanceledException, httpStatus 0 — nivel exacto no distinguible desde adentro). Refuerza la regla de **≥2 TSAs configuradas con fallback** (ADR-005) — el fallback ya demostró su valor en la primera corrida real |
| Ejecución del stack dentro del **sandbox real de Dataverse** | **RESUELTO POR SPIKE (2026-07-15) — PASS** | ImageSharp ✔ (PNG RGBA 1.470 B en 4 ms), QRCoder ✔ (584 B), PDF vía XObject manual ✔, SHA-256 ✔, TSA Sectigo ✔. isolationMode=2, runtime 4.0.30319. Bonus: `dotnet pack` del plugin package funciona 100% en Linux |
...
- System.Text.Json 9.0.6: nuget.org/packages/System.Text.Json/9.0.6 (assets `lib/net462` confirmados) · validado en sandbox real (spike v1.0.9, §10) · finding de unificación de versión por la plataforma: `spikes/spike-sandbox/RESULTADOS-SANDBOX.md`
