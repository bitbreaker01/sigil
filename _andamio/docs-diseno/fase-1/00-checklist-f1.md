...
| F2.5 | Script de carrera de locks contra Dev (doc 11 §3) | Claude + Usuario | N `SubmitSignature` concurrentes → exactamente un Sellando | ✅ 2026-07-16 — `tests/integration/Sigil.LockRace` (impersonación por firmante + barrera para solapar). **PASA contra Dev:** escenario A (3 firmas simultáneas → `IsLastSigner`×1, 1 ledger, 3 Firmado, Completado) + escenario B (doble click → idempotente, tx queda Firmado Parcialmente). Antagonista aplicado: la prueba REAL del lock es `IsLastSigner==1` (el assert de ledger valida el alternate key, no el lock — el alternate key lo enmascararía); B verifica que la tx NO transiciona; barrera arreglada (RemoveParticipant solo pre-barrera); límite declarado (una instancia a la vez). CI: `lock-race-harness` compila en cada PR. **→ F2 (BACKEND) COMPLETA** |

> **✅ F2 (BACKEND) COMPLETA (2026-07-16):** 16 Custom APIs + worker asíncrono desplegados en Dev, **177 Core + 147 conformidad verdes**, el ciclo probatorio completo validado E2E (CF-D10: sellar con TSA real; CF-D11: verificar Verde/Rojo) y la serialización de locks validada bajo concurrencia real (F2.5). Sigue **F3 — el frontend (Power Apps Code App)**.
